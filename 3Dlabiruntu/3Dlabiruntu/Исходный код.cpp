...
void display()
{
	...
	for (int i = 0; i<map.x; i++)
		...
		drawBox(j, i);
			...
			/*char d[m + 2][m + 2];     //просто ыыцыалызував порожный масив
			for (int i = 0; i < 42; i++)
				for (int j = 0; j < 42; j++){
				d[i][j] = '0';
				}
			...
			glPushMatrix();        //це взагал≥ не рухаЇм бо то довго мучивс€
			glMatrixMode(GL_PROJECTION);
			glPushMatrix();
			glLoadIdentity();
			glOrtho(0, 100, 0, 100, -25, 25);//задаем проекцию
			glMatrixMode(GL_MODELVIEW);
			glLoadIdentity();
...
//ось це функцы€ промальовки карти
void drawCompas(){
